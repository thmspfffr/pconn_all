...
%   cvar_res_all = double(cvar_res_all(:,:,SUBJLIST,:));
  ...
  for nsubj = SUBJLIST
    ...
    for i = 1 : 4
      ...
      par = nanmean(nanmean(nanmean(all_par_res(:,FOI_cvar(i,1):FOI_cvar(i,2),:,:),2),3),4);
...
load(sprintf('~/pconn_all/proc/all_sens_res_%s_clusterstat_contr%d_f%d_v%d.mat','dfa',1,ifoi,2));

avg_res = squeeze(all_par_res(:,:,:,ifoi));
avg_cnt = squeeze(all_par_cnt(:,:,:,ifoi));
