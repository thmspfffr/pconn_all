...
%         
        if ~exist(sprintf([outdir 'pconn_sens_dfa_filt_s%d_m%d_f%d_v%d_processing.txt'],isubj,m,ifoi,v))
          system(['touch ' outdir sprintf('pconn_sens_dfa_filt_s%d_m%d_f%d_v%d_processing.txt',isubj,m,ifoi,v)]);
        else
          continue
        end
        % %
        ...
        if (isubj~=3 && isubj~=2 &&  isubj~=17) && isubj <= 24
          d = dir(sprintf('/home/tpfeffer/pconn/proc/preproc/pconn_postproc_s%d_m%d_b*_v%d.mat',isubj,m,v_rawdata));
        else
          d = dir(sprintf('/home/tpfeffer/pconn/proc/preproc/pconn_postproc_s%d_m%d_b*_v%d.mat',isubj,m,3));
        end
        ...
        for iblock = 1 : length(d)
          ...
          tmp     = tp_filt(mydata(:,1),[foi(ifoi,1) foi(ifoi,2)],fsample);
          ampenv  = abs(hilbert(zscore(tmp))); clear tmp mydata
          
          % second filter
          ampenv    = zscore(tp_filt(ampenv,[0.01 2],fsample,10,'ord',1,'meth','but'));
        ...
        save(sprintf([outdir 'pconn_sens_dfa_filt_s%d_m%d_f%d_v%d.mat'],isubj,m,ifoi,v),'par','-v7.3');
        clear par
